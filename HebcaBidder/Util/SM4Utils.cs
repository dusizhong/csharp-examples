...
namespace HebcaBidder.Util
{
    public class SM4Utils
    {
        public SM4Utils()
        {
            Key = "1234567890123456"; //加密密钥，长度必须为16字节
            Iv = "0000000000000000"; //CBC向量
            HexString = false; //加密密钥是否转为16进制
            CryptoMode = Sm4CryptoEnum.ECB; //加密模式ECB CBC
        }
        ...
        public static string EncryptECB(string secretKey, byte[] plainText)
        {
            ...
            SM4 sm4 = new SM4();
            sm4.SetKeyEnc(ctx, Encoding.UTF8.GetBytes(secretKey));
        ...
        public static string DecryptECB(string secretKey, string cipherText)
        {
            ...
            SM4 sm4 = new SM4();
            sm4.Sm4SetKeyDec(ctx, Encoding.UTF8.GetBytes(secretKey));
            byte[] decrypted = sm4.Sm4CryptEcb(ctx, Hex.Decode(cipherText));
            return Encoding.UTF8.GetString(decrypted);
        }

        // 未修改，暂未使用
        public static string EncryptCBC(SM4Utils entity)
        {
            Sm4Context ctx = new Sm4Context
            {
                IsPadding = true
        ...
        // 未修改，暂未使用
        public static string DecryptCBC(SM4Utils entity)
        {
            Sm4Context ctx = new Sm4Context
            {
                IsPadding = true,
    ...
    internal class SM4
    {
        ...
        public byte[] Sm4CryptCbc(Sm4Context ctx, byte[] iv, byte[] input)
        {
            ...
            byte[] bins = new byte[length];
            Array.Copy(input, 0, bins, 0, length);
            System.Collections.Generic.List<byte> bousList = new List<byte>();
            int i;
            if (ctx.Mode == 1)
            {
